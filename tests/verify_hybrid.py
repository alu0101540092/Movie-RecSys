...
def verify():
    ...
    recs_horror = get_recommendations(user_id, n=5, selected_genres=["Horror"], alpha=0.0)
    
    horror_count = 0
    for r in recs_horror:
        print(f"{r['title']} ({r['genres']}) - Hybrid Score: {r['hybrid_score']:.2f}")
    ...
    recs_balanced = get_recommendations(user_id, n=5, selected_genres=["Horror"], alpha=0.5)
    for r in recs_balanced:
        print(f"{r['title']} ({r['genres']}) - SVD: {r['score']:.2f}, Hybrid: {r['hybrid_score']:.2f}")
