...
def analyze():
    ...
    animation_movies = movies_df[
        movies_df["genres"].str.contains("Animation", na=False)
    ].head(5)
    ...
    for _, row in animation_movies.iterrows():
        ...
        if iid is None:
            iid = mappings["items"].get(str(mid))
        if iid is not None:
            vector_indices.append(iid)
    ...
    user_factors, user_bias = fold_in_user(
        user_ratings_df, qi, bi, global_mean, mappings, n_epochs=20, lr=0.005
    )
    ...
    for i in top_indices:
        ...
        if mid:
            ...
            if not row.empty:
                print(
                    f"  - {row.iloc[0]['title']} ({row.iloc[0]['genres']}) [Score: {scores[i]:.4f}]"
                )
