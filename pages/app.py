...
def run_eval(cv: int, measures: tuple[str, ...], verbose: bool):
...
results, results_mean, results_std, df_long = run_eval(
    cv=cv, measures=tuple(chosen_measures), verbose=verbose
)

# Tabs por métrica de calidad seleccionada
...
for t, metric in zip(tabs, chosen_measures):
    with t:
        ...
        # Ordena según si menor es mejor (RMSE/MSE/MAE) o mayor es mejor (FCP)
        asc = metric in ("RMSE", "MSE", "MAE")
        df_plot = df_plot.sort_values("mean", ascending=asc)

        # Barra con error (Altair)
        bars = (
            alt.Chart(df_plot)
