...
def optimize():
    ...
    if hasattr(algo, 'trainset') and hasattr(algo.trainset, 'global_mean'):
        np.save(os.path.join(models_dir, 'svd_global_mean.npy'), np.array([algo.trainset.global_mean]))
        print(f"Saved svd_global_mean.npy: {algo.trainset.global_mean}")
    elif hasattr(algo, 'default_prediction'): # Fallback
         np.save(os.path.join(models_dir, 'svd_global_mean.npy'), np.array([algo.default_prediction]))
         print(f"Saved svd_global_mean.npy (from default_prediction): {algo.default_prediction}")
