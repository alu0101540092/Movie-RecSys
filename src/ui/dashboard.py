    ...
    if results.empty:
        ...
        for index, row in results.iterrows():
            with st.expander(f"{row['title']} ({row['genres']})"):
                # Rating slider
                rating = st.slider("Tu valoración", 1.0, 5.0, 3.0, 0.5, key=f"rate_{row['movieId']}")
                if st.button("Enviar Valoración", key=f"btn_{row['movieId']}"):
                    add_rating(st.session_state['user_id'], row['movieId'], rating)
                    st.success(f"Valoraste '{row['title']}' con {rating} estrellas.")
...
def render_recommendations_tab():
    ...
    if st.button("Generar Recomendaciones"):
        ...
        if not recs:
            st.info("No hay suficientes datos para generar recomendaciones. ¡Valora algunas películas primero!")
        else:
            for rec in recs:
...
def render_profile_tab():
    ...
    if not user_ratings.empty:
        movies_df = load_movies()
        # Merge to get titles
        merged = user_ratings.merge(movies_df, left_on='movie_id', right_on='movieId')
